...
def build_and_train_models(X_train, y_train, n_estimators=100, learning_rate=1.0, max_depth=5):
    """构建并训练AdaBoost、随机森林和单棵决策树模型"""
    print("\n" + "=" * 60)
    print("步骤3：构建并训练模型")
    print("=" * 60)
    ...
    return ada_model, rf_model, dt_model

def evaluate_models(ada_model, rf_model, dt_model, X_test, y_test, cancer):
    """评估模型性能"""
    print("\n" + "=" * 60)
    print("步骤4：模型预测与评估")
    print("=" * 60)
    ...
    print(f"\n模型评估结果：")
    print(f"\nAdaBoost：")
    print(f"  准确率 (Accuracy): {ada_accuracy:.4f}")
    print(f"  F1分数 (F1-Score): {ada_f1:.4f}")
    ...
    return (ada_pred, ada_accuracy, ada_f1,
            rf_pred, rf_accuracy, rf_f1,
            dt_pred, dt_accuracy, dt_f1)

def visualize_results(ada_accuracy, rf_accuracy, dt_accuracy,
                      ada_f1, rf_f1, dt_f1):
    """可视化结果"""
    print("\n" + "=" * 60)
    print("步骤5：结果可视化")
    ...
    axes[0].set_title('三种模型性能对比', fontsize=14, fontweight='bold')
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(models)
    axes[0].legend()
    axes[0].set_ylim(0.9, 1.0)  # 调整y轴范围以突出差异
    ...
    # 2. 绘制性能提升对比图
    # 计算相对于单棵决策树的提升
    dt_baseline_acc = dt_accuracy
    dt_baseline_f1 = dt_f1

    rf_acc_improvement = (rf_accuracy - dt_baseline_acc) * 100
    rf_f1_improvement = (rf_f1 - dt_baseline_f1) * 100
    ada_acc_improvement = (ada_accuracy - dt_baseline_acc) * 100
    ada_f1_improvement = (ada_f1 - dt_baseline_f1) * 100
    ...
    axes[1].set_ylabel('提升百分比 (%)', fontsize=12)
    axes[1].set_title('相对于单棵决策树的性能提升', fontsize=14, fontweight='bold')
    ...
    print(f"\n结果图已保存为 'task4_results.png'")

def run_experiment():
    """运行完整实验"""
    print("\n" + "=" * 80)
    print("任务4：AdaBoost分类（基于乳腺癌数据集）")
    print("=" * 80)
    ...
    # 3. 构建并训练模型
    ada_model, rf_model, dt_model = build_and_train_models(X_train, y_train)

    # 4. 评估模型
    (ada_pred, ada_accuracy, ada_f1,
     rf_pred, rf_accuracy, rf_f1,
     dt_pred, dt_accuracy, dt_f1) = evaluate_models(
        ada_model, rf_model, dt_model, X_test, y_test, cancer
    )

    # 5. 可视化结果
    visualize_results(ada_accuracy, rf_accuracy, dt_accuracy,
                      ada_f1, rf_f1, dt_f1)
    ...
    print(f"\n2. 关键发现：")
    print(f"   - 集成学习方法（随机森林和AdaBoost）通常优于单棵决策树")
    print(f"   - AdaBoost和随机森林各有优势，性能相近")
    print(f"   - 集成学习能够有效提升模型性能")

    print(f"\n3. 模型优势：")
    print(f"   - AdaBoost能够自动调整样本权重")
    print(f"   - 随机森林能够并行训练")
    print(f"   - 两种方法都能有效提升模型性能")

    print(f"\n4. 潜在改进方向：")
    print(f"   - 调整学习率和弱学习器数量参数")
    print(f"   - 使用交叉验证选择最优参数")
    print(f"   - 尝试其他集成方法（如梯度提升树）")

    return ada_model, ada_accuracy, ada_f1

if __name__ == "__main__":
    ada_model, ada_accuracy, ada_f1 = run_experiment()
